...
public class InterCityBusList extends ListActivity{
	...
	private void populateList() {
		...
		ArrayList<Bus> buss = dbHelper.getInterCityBusList();
		list.clear();
		for (Bus bus : buss) {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
			Intent intentMain = new Intent(InterCityBusList.this, InterCityBusRoute.class);
			int busId=Integer.parseInt(list.get(position).get("id"));
			intentMain.putExtra("busId", busId);
			InterCityBusList.this.startActivity(intentMain);
		} 
		catch (Exception e) {
